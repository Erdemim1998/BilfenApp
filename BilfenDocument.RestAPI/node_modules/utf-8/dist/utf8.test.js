...
describe('Decoding chars should work', () => {
    ...
    test('for 4 bytes encoded chars in a TypedArray', () => {
        // �    U+10000    65536    (11110000 10010000 10000000 10000000)  (F0 90 80 80)
        expect(UTF8.getCharCode(new Uint8Array([0xf0, 0x90, 0x80, 0x80]))).toEqual(65536);
        // []    U+10FFFF  1114111  (11110100 10001111 10111111 10111111)  (F4 8F BF BF)
        expect(UTF8.getCharCode(new Uint8Array([0xf4, 0x8f, 0xbf, 0xbf]))).toEqual(1114111);
        // []    U+1D306  119558  (11110000 10011101 10001100 10000110)  (F0 9D 8C 86)
        expect(UTF8.getCharCode(new Uint8Array([0xf0, 0x9d, 0x8c, 0x86]))).toEqual(119558);
    });
});
...
describe('Encoding then decoding strings should work', () => {
    test('for this simple sentence', () => {
        expect(UTF8.getStringFromBytes(['H'.charCodeAt(0), 'e'.charCodeAt(0)])).toEqual('He');
    });
    test('for this simple sentence', () => {
        expect(UTF8.getStringFromBytes(UTF8.setBytesFromString("J'avais gagné 15€ au tiercé !"))).toEqual("J'avais gagné 15€ au tiercé !");
    });
...
describe('Encoding unencodable chars', () => {
    test('should raise an exception', async () => {
        try {
            ...
            expect(err.code).not.toEqual('E_UNEXPECTED_SUCCESS');
        }
    });
});
...
describe('Decoding unterminated strings', () => {
    ...
    test('should raise an exception in strict mode', async () => {
        try {
            UTF8.getStringFromBytes(new Uint8Array([0xf4, 0x8f]), undefined, undefined, true);
            throw new YError('E_UNEXPECTED_SUCCESS');
        }
        catch (err) {
            expect(err.code).not.toEqual('E_UNEXPECTED_SUCCESS');
...
describe('Encoding strings in a too small ArrayBuffer', () => {
    test('should silently fail', () => {
        ...
        expect(UTF8.getStringFromBytes(UTF8.setBytesFromString('1.3$ ~= 1€', bytes))).toEqual('1.');
        expect(UTF8.getStringFromBytes(UTF8.setBytesFromString('€', bytes))).toEqual('');
        expect(UTF8.getStringFromBytes(UTF8.setBytesFromString('1é', bytes))).toEqual('1');
    });
    test('should fail when using strict mode', () => {
        try {
            ...
            expect(err.code).toEqual('E_BUFFER_OVERFLOW');
            expect(err.params).toEqual(['3', 2]);
        }
        try {
            const bytes = new Uint8Array(2);
...
describe('Detecting the encoding', () => {
    ...
    test('should fail with non UTF-8', () => {
        expect(UTF8.isNotUTF8(new Uint8Array([0xc0, 0x90, 0x80, 0x01]))).toEqual(true);
    });
});
